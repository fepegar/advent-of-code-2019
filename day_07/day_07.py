...
class Amplifier:
    ...
    def get_outputs(self, phase, input_):
        return self.computer.run(program_inputs=[phase, input_])
    ...
    for combination in permutations(range_phases, num_amps):
        ...
        while not found:
            for amp, phase in zip(amps, combination):
                amp_outputs = amp.get_outputs(phase, amp_input)
                if not amp_outputs:
                    found = True
                    break
...
print(get_max_signal(program, part=2))
